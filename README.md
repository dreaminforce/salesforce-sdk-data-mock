...
└── force-app/main/default/uiBundles/
    └── MyReactApp/                                   # UI Bundle app folder
        ...
        ├── package-lock.json
        ├── vite.config.ts                            # Add the mock plugin here
        └── src/
```
...
### Keep the Generated Aliases Unchanged

Do not edit `resolve.alias`. `salesforceMockDataPlugin()` automatically adds exact-match aliases for `@salesforce/platform-sdk/data` and `@salesforce/platform-sdk` when the plugin is enabled. Vite merges them with every alias already generated by Salesforce, including aliases Salesforce may add in future project templates.

The exact matching prevents the mock from intercepting unrelated Platform SDK modules such as `@salesforce/platform-sdk/chat`. Normal `npm run dev` behavior is unchanged because the plugin is only included in mock mode.

If you are upgrading from `0.2.0` and previously added the two mock aliases manually, remove only those two `@salesforce/platform-sdk` entries from `resolve.alias`. Keep every project alias generated by Salesforce. It is safe to leave those project aliases in array form, or you can restore Salesforce's original object form.
...
- `salesforceMockDataPlugin()` is included in the Vite `plugins` array in mock mode
- You did not add or modify SDK aliases manually; the plugin manages them
...
- development dependencies: `typescript` and `vite` (for integration tests)
